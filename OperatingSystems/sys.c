...
#endif
SYSCALL_DEFINE1(sena, char *, msg)
{
  char buf[256];
  long copied = strncpy_from_user(buf, msg, sizeof(buf));
/*not: copy_from_user ile strncpy_from_user benzerler: userspace den kernel space’e data kopyalaniyor… Tam tersini copy_to_user ile yapabiliriz
*/
  if (copied < 0 || copied == sizeof(buf))
    return -EFAULT;
  printk(KERN_INFO "sena syscall called with \"%s\"\n", buf);
  return 0;
}
